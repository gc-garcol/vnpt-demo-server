...
/**
 * REST controller for managing {@link gc.garcol.todoapp.domain.Card}.
 */
@RestController
@RequestMapping("/api")
public class CardResource {
    ...
    public CardResource(CardService cardService, CardRepository cardRepository, TaskRepository taskRepository) {
    ...
    @GetMapping("/tasks/{task-id}/cards")
    public List<CardDTO> getAllCardsByTaskId(@PathVariable("task-id") Long taskId) {
        log.debug("REST request to get all Cards");

        if (!taskRepository.existsById(taskId)) {
            throw new BadRequestAlertException("Entity not found", "task", "idnotfound");
        }

        return cardService.findAll(taskId);
    }
